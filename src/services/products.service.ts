...
class ProductsService{
    public async addServiceProducts(reqBody: IProducts){
  ...
  public async UpdateServiceProduct(idproduc: number, reqBody: IProducts) {
    const product = await Products.findOneBy({ idproduc: idproduc });
  ...
  public async deleteServiceProduct(idproduc: number) {
    const product = await Products.findOneBy({ idproduc: idproduc });
    if (!product) {
      return Promise.reject("No existe Producto");
    } else {
        product.state = 0;
        product.save();
      return product;
